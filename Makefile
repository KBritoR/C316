.PHONY: install run test test-cov help docker-up docker-down docker-build docker-logs docker-restart
...
test-cov:
	cd $(BACKEND_DIR) && $(POETRY) run pytest -v --cov=. --cov-report=term-missing
